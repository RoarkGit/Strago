...
export const register: Command = {
  data: new SlashCommandBuilder()
    .setName('register')
    .setDescription(
      'Register a character with Strago for achievement scanning.',
    )
    .addStringOption((option) =>
      option.setName('character').setDescription("Your character's name."),
    )
    .addStringOption((option) =>
      option
        .setName('server')
        .setDescription(
          'The server (e.g. Balmung) on which your character resides.',
        )
    ...
    .addStringOption((option) =>
      option
        .setName('id')
        .setDescription(
          "Your character's Lodestone ID. Use this if name search fails.",
        ),
    ),
  run: async (
    ...
    try {
      ...
      if (
        ...
        await interaction.editReply({
          content: 'I was unable to locate that character.',
        })
        return
      }
      ...
      const message = await interaction.editReply({
        content: [
          `I found this character: <${xivlib.getUrl([characterId])}>`,
      ...
      message
        ...
        .then(async (i) => {
          if (
            characterId != null &&
            (await xivlib.verifyCharacter(characterId))
          ) {
            ...
            await Character.findOneAndReplace(
              { discordId: i.user.id },
            ...
            await interaction.editReply({
              content:
                'I could not verify the challenge on your Lodestone profile.',
              components: [],
            })
          }
        })
